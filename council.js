...
async function main () {
    ...
    const unsubscribe = await api.rpc.chain.subscribeNewHeads(async (header) => {
        ...
        if (councilstage == null) {
            console.log('Council has been elected')
            if (block>lastcouncilnotif){
                bot.sendMessage(chatid, `New council has been elected at block ${lastcouncilnotif}.`, { parse_mode: 'html' })
                lastcouncilnotif=lastcouncilnotif+councilperiod
            }
        }
    })
}
